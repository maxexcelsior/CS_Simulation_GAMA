/***
* Name: segregplusdemand
* Author: mirei
* Description: 
* Tags: Tag1, Tag2, TagN
***/

model segregplusdemand
...
global{
	...
	action countRent{
		...
		if(cycle!=0){
			increasedBuildingMeanRent <- (meanRentBuilding-(mean(building where(each.usage = "R") collect each.rentPriceNorm)))*100;
		}
...
//parameter "Number of people" var: nb_people <- 1500 min:10 max:10000 category: "Population";
	output{
		...
		display Charts2{
			...
			chart "Max rent" type:series background: # white position:{0,0.50} size:{1.0,0.25}{
