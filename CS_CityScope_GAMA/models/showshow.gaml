...
global{
	...
	file resuls_40000 <- file("../results/results_segreg40000.csv");
	file resuls_100000 <- file("../results/results_segreg100000.csv");
	...
	init{
		...
		//do import_resuls_40000;
		//do import_resuls_50000;
		do import_resuls_100000;
	}
	...
	action import_resuls_100000{
		matrix resuls_matrix <- matrix(resuls_100000);
		loop i from: 0 to: resuls_matrix.rows - 1{
			float location_x <- resuls_matrix[3,i];
			float location_y <- resuls_matrix[4,i];
			string people_type <- resuls_matrix[1,i];
	...
	/***action import_resuls_50000{
		matrix resuls_matrix <- matrix(resuls_50000);
		loop i from: 0 to: resuls_matrix.rows - 1{
			float location_x <- resuls_matrix[3,i];
			float location_y <- resuls_matrix[4,i];
			string people_type <- resuls_matrix[1,i];
...
experiment visual type:gui{
	...
	output{
		...
		display map4 type: opengl draw_env: false background: #black {
			species building aspect: default;
			species road;
			species people4 aspect: default;
